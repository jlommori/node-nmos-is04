...
class NodeAPI extends EventEmitter {
  constructor(port, store, iface) {
    super()
    this.app = express();
    this.port = port;
    if (!port) this.port = '3000'
    this.store = store;
    this.iface = iface;
    if (!iface) this.iface = "0.0.0.0"
    this.server = null;
    ...
    this.app.get(`${base}/`, rootResult)

    //TODO: REST API endpoints for NodeAPI
  ...
  start(node) {
    let host = this.iface
    let port = this.port
    this.server = this.app.listen(this.port, this.iface, function (e) {
      if (e) {
        ...
        console.log('Node NMOS-IS04 Node server running at http://%s:%s', host, port);
      };
    });
  ...
  startMDNS(node) {
    ...
    if (!mdnsService) {
      ...
      mdnsService = mdns.createAdvertisement(mdns.tcp('nmos-node'), 3000, {
        name : hostname,
...
//     if (pageOf) res.set('X-NMOS-IS04-PageOf', pageOf.toString());
//     if (size) res.set('X-NMOS-IS04-Size', size.toString());
//     if (pages) res.set('X-NMOS-IS04-Pages', pages.toString());
//     if (total) res.set('X-NMOS-IS04-Total', total.toString());
//     return res;
//   }
//
//   /**
